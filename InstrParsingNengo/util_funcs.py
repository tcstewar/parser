...
class MirroredFilteredStepFunction(Function):
    def __init__(self, index = 0, scale = 20, shift = 0, step_val = 1, on_center = False):
        # on_center - True =>  _/\_
        #             False => \__/
        self.scale = scale
        self.shift = shift
    ...
    def map(self, func_in):
        func_val = func_in[self.index]
        mirror_val = 1 - 2*self.on_center
        return max(-1/exp(mirror_val * (func_val-self.shift) * self.scale) + 1, 0) * self.step_val * (func_val >= 0 ) + \
               max(-1/exp(mirror_val * (-func_val-self.shift) * self.scale) + 1, 0) * self.step_val * (func_val < 0)
...
class VFunction(Function):
    def __init__(self, dead_zone = 0.0, radius = 1.0, index = 0):
        self.index = index
        self.dead_zone = dead_zone
        self.radius = radius

    def map(self, func_in):
        func_val = abs(func_in[self.index])
        if( func_val <= self.dead_zone ):
            return 0
        else:
            return (func_val - self.dead_zone) / (self.radius - self.dead_zone)
...
def ew_sum(*args):
    ...
    if( len(args) == 1 ):
        ...
        for item in args:
            len2 = len(item)
            if( len1 != len2 ):
                print("util_funcs.ew_sum - Mismatching vector lengths, returning 0")
                return 0
            result = [result[n] + item[n] for n in range(len2)]
        return result
#    old_len = args[0]
#    for item in args:
#        old_len = len(
...
def vec_scale(item1, scale):
    return [n * scale for n in item1]
